package day05_Concatenation;

public class CarInfo {
    public static void main(String[] args) {
int year=2010;
String make="Mazda";
String model="3";
int kilometers=174000;
String color="light Blue";
int price=6000;
        System.out.println(year+" "+make+" "+model+" ,"+kilometers+" km ,"+color+", $"+price+".");

    }
}
/*
1. Create a class calledCarInfo.java 2. Declare the following variables:
1. year 2. make
3. model 4. miles 5. color 6. price
3. Use concatenation to print the full info of the car in the following format:
Year Make Model, Miles, Color, Price. 2018 Toyota Camry, 50000 miles, Red, $19000.
 */